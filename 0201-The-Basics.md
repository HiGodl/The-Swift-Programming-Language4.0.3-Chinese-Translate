...
错误抛出、捕获、传递在[错误处理](0201-The-Basics.md#errorHandling)章节有详细介绍
...
除了定位运行时错误，断言与前置条件也是一种非常有用的代码文档的一种形式。与[错误处理](0201-The-Basics.md#errorHandling)不同的是，断言与前置条件并不是用来处理可恢复的预期的错误。由于断言与前置条件是是一种非法的程序运行状态，所以没有办法捕获到失败的断言。
