...
>注意
>> 除非使用到与运行平台同位宽的无符号整型值的情况下使用 `UInt` ，一般情况下就算只需要存储非负数也推荐使用 `Int`。使用`Int`是为了保证代码的通用性，不必在两种数值类型间进行转换。整型匹配推断，在 [类型安全和类型推断](ji-chu.md#Type_Safety_and_Type_Inference) 中有详细介绍。
...
####<span id="Type_Safety_and_Type_Inference">类型安全和类型推断 </span>
