...
这种操作被成为隐式解析可选类型。通过在类型之后加一个感叹号`!`的方式来定义隐式解析可选类`(String!)`。

当可选类型在定义之后就有值并且在之后的任何时候都有值，使用隐式解析可选类型会更方便。在`Swift`中隐式解析可选类型主要用在类的初始化方法中，在 [非持有引用类型和隐式解包可选类型属性]() 章节中有详细说明。
